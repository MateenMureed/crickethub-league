...
export { default } from './components/GraphicsGeneratorPanel'
...
async function _generateTeamBanner(teamId) {
  ...
  const titleLine2 = (team.event || 'FOR ICC MEN\'S T20 WORLD CUP').toUpperCase()
...
async function _generateInningsBanner(match, scorecard, type) {
  ...
  return { canvas, id: match.id, name: 'innings_banner' }
}
...
async function _generateSummaryBanner(match, scorecard, leagueObj) {
  ...
  const matchDate    = match.match_date   ? new Date(match.match_date).toLocaleDateString('en-GB', { day:'numeric', month:'short', year:'numeric' }) : ''
...
async function _finish(result, category) {
...
export async function generateSquadBannerForTeam(teamId) {
  const result = await _generateTeamBanner(teamId)
  return _finish(result, 'teams')
}

export async function generateCaptainPosterForTeam(teamId) {
  const result = await _generateCaptainPoster(teamId)
  return _finish(result, 'teams')
}

export async function generateLeagueBannerForLeague(leagueId) {
  const result = await _generateLeagueBanner(leagueId)
  return _finish(result, 'leagues')
}

export async function generateVsBannerForMatch(match, leagueObj) {
  const result = await _generateVsBanner(match, leagueObj)
  return _finish(result, 'matches')
}

export async function generateInningsBannerForMatch(match, scorecard, type) {
  const result = await _generateInningsBanner(match, scorecard, type)
  return _finish(result, 'matches')
}

export async function generateResultBannerForMatch(match, scorecard) {
  const result = await _generateResultBanner(match, scorecard)
  return _finish(result, 'results')
}

export async function generateSummaryBannerForMatch(match, scorecard, leagueObj) {
