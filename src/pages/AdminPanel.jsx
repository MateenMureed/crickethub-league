...
export default function AdminPanel() {
  ...
  const SectionHead = ({ title, action }) => (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14, gap:10 }}>
      <h2 style={{ fontSize:'1rem', fontWeight:900, margin:0 }}>{title}</h2>
      {action}
    </div>
  )
  ...
  const BannerSection = ({ children }) => (
    <div style={{ marginTop:10, paddingTop:10, borderTop:'1px solid var(--glass-bd)', display:'flex', flexWrap:'wrap', gap:5, alignItems:'center' }}>
      <span style={{ fontSize:'0.6rem', fontWeight:700, color:'var(--gold)', letterSpacing:'1px', textTransform:'uppercase' }}>🎨</span>
      {children}
    </div>
  )
      ...
      <div className="admin-content">
        ...
        {activeSection === 'leagues' && (
          <div>
            <SectionHead
              title="Leagues"
              action={<button className="btn btn-primary btn-sm" onClick={() => setShowModal('createLeague')}>+ Create</button>}
            />
            {selectedLeagueObj && (
              <div className="glass-card" style={{ padding:'12px 14px', marginBottom:10, borderLeft:'3px solid var(--gold)' }}>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, marginBottom:6 }}>
                  <div style={{ fontWeight:800, fontSize:'0.86rem' }}>League Details</div>
                ...
                <div style={{ fontSize:'0.75rem', color:'var(--t2)', lineHeight:1.5 }}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                    {teams.map(team => (
                      <div key={team.id} className="glass-card" style={{
                        padding:'11px 14px', cursor: inlineEditTeamId === team.id ? 'default' : 'pointer',
                        ...
                        {inlineEditTeamId === team.id ? (
                          <div style={{ display:'flex', flexDirection:'column', gap:8 }} onClick={e => e.stopPropagation()}>
                            <input className="form-input" value={inlineTeamDraft.name} onChange={e => setInlineTeamDraft(p => ({ ...p, name:e.target.value }))} placeholder="Team Name" />
                            <input className="form-input" value={inlineTeamDraft.captain_name} onChange={e => setInlineTeamDraft(p => ({ ...p, captain_name:e.target.value }))} placeholder="Captain Name" />
                            <div className="form-grid">
                              <input className="form-input" type="file" accept="image/*" onChange={e => setInlineTeamLogo(e.target.files[0] || null)} />
                              <input className="form-input" type="file" accept="image/*" onChange={e => setInlineCaptainPhoto(e.target.files[0] || null)} />
                            </div>
                            <div style={{ display:'flex', gap:6 }}>
                              <button className="btn btn-sm btn-primary" onClick={() => saveInlineTeamEdit(team.id)}>Save</button>
                              <button className="btn btn-sm btn-secondary" onClick={cancelInlineTeamEdit}>Cancel</button>
                            </div>
                          </div>
                        ) : (
                        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <form onSubmit={createLeague}>
              <div className="modal-body">
                ...
                <div className="form-grid">
                  ...
                  <div className="form-group"><label className="form-label">Overs / Innings</label><input className="form-input" type="number" min="1" value={leagueForm.overs_per_innings} onChange={e => setLeagueForm({...leagueForm,overs_per_innings:e.target.value})} /></div>
                </div>
